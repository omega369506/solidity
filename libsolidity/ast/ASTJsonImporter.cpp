...
ASTPointer<ContractDefinition> ASTJsonImporter::createContractDefinition(Json const& _node)
{
	...
	return createASTNode<ContractDefinition>(
		...
		createNameSourceLocation(_node),
		_node.contains("documentation") ? (_node["documentation"].is_null() ? nullptr : createDocumentation(member(_node, "documentation"))) : nullptr,
		baseContracts,
		subNodes,
		contractKind(_node),
...
ASTPointer<UsingForDirective> ASTJsonImporter::createUsingForDirective(Json const& _node)
{
	...
	return createASTNode<UsingForDirective>(
		...
		_node.contains("typeName") ? (_node["typeName"].is_null() ? nullptr  : convertJsonToASTNode<TypeName>(_node["typeName"])) : nullptr,
...
ASTPointer<FunctionDefinition> ASTJsonImporter::createFunctionDefinition(Json const&  _node)
{
	...
	return createASTNode<FunctionDefinition>(
		...
		_node.contains("overrides") ? (_node["overrides"].is_null() ? nullptr : createOverrideSpecifier(member(_node, "overrides"))) : nullptr,
		_node.contains("documentation") ? (_node["documentation"].is_null() ? nullptr : createDocumentation(member(_node, "documentation"))) : nullptr,
...
ASTPointer<VariableDeclaration> ASTJsonImporter::createVariableDeclaration(Json const& _node)
{
	...
	return createASTNode<VariableDeclaration>(
		...
		visibility(_node),
		_node.contains("documentation") ? (_node["documentation"].is_null() ? nullptr : createDocumentation(member(_node, "documentation"))) : nullptr,
		_node.contains("indexed") ? (_node.contains("indexed") ? memberAsBool(_node, "indexed") : false) : false,
		mutability,
		_node.contains("overrides") ? (_node["overrides"].is_null() ? nullptr : createOverrideSpecifier(member(_node, "overrides"))) : nullptr,
		location(_node)
	);
...
ASTPointer<ModifierDefinition> ASTJsonImporter::createModifierDefinition(Json const&  _node)
{
	return createASTNode<ModifierDefinition>(
		...
		memberAsBool(_node, "virtual"),
		_node.contains("overrides") ? (_node["overrides"].is_null() ? nullptr : createOverrideSpecifier(member(_node, "overrides"))) : nullptr,
		_node.contains("body") ? (_node["body"].is_null() ? nullptr: createBlock(member(_node, "body"), false)) : nullptr
	);
}
...
{
	return _json.contains(_name) ? (_json[_name].is_string() ? memberAsASTString(_json, _name) : nullptr) : nullptr;
}
